...
class TimeRange(BaseModel):
    ...
    time_start: Optional[str] = Field(
        None, description="Start time HH:MM, e.g. '08:00'"
    )
...
class CanonicalRule(BaseModel):
    ...
    # ── Identity ──────────────────────────────────────────────────────────────
    rule_name: str = Field(
        description="Short machine-friendly name, e.g. 'Block_SSH_Finance'"
    )
    ...
    source_is_any: bool = Field(
        False, description="True when source is literally 'any'"
    )
    destination_is_any: bool = Field(
        False, description="True when destination is 'any'"
    )

    # ── Action & direction ────────────────────────────────────────────────────
    action: Action = Field(description="permit, deny, or reject")
    direction: Direction = Field(
        Direction.INBOUND, description="inbound or outbound on the interface"
    )
    ...
    incomplete: bool = Field(
        False, description="True when any entity is Not Found or confidence < 0.5"
    )
    ...
    def validate_rule(self) -> "CanonicalRule":
        ...
        if not self.destination_is_any and len(self.destinations) == 0:
            raise ValueError(
                "destinations must be non-empty, or destination_is_any=True"
            )
